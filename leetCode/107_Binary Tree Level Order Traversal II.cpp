...
int** levelOrder(struct TreeNode* root, int** columnSizes, int* returnSize) {
    ...
    int low = 0, high = res->returnSize - 1;
    int *tmp_ans, tmp_col;
    while(low < high) {
        tmp_ans = res->ans[low];
        res->ans[low] = res->ans[high];
        res->ans[high] = tmp_ans;
        tmp_col = res->columnSizes[low];
        res->columnSizes[low] = res->columnSizes[high];
        res->columnSizes[high] = tmp_col;
        ++low;
        --high;
    }
